...
export class UserDetailService {
    ...
    doReg(userDetails: IRegisterDetails): Observable<IRegisterDetails> {
        return this.http.post<IRegisterDetails>('http://localhost:9090/users/createUser', userDetails);
    }
    doLogin(loginDetails: ILoginDetails): Observable<ILoginDetails> {
        return this.http.post<ILoginDetails>('http://localhost:3000/profiles', loginDetails);
    }
